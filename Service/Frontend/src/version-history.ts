...
const versionDetails = reactive<DataObject>({
  ...
  "20250928_v0.4.0": [
    "[資產管理] 超商實收清單 - 顯示 Bug 修正",
    "[員工管理] 員工業績列表 - 標題顯示遊戲名稱",
    "[系統設定] 使用權限 - 新增返回功能",
    "[系統設定] 線上監控 - 新增返回功能",
    "[系統設定] 交流方式 - 刪除改至交流敘述右方，並且點擊直接刪除",
    "[交流單管理] 新增交流單 - 新增拆帳欄位、拆帳金額",
    "[拆帳管理] 拆帳列表 - 新增待拆帳清單及拆帳功能",
    "[資產管理] 交流紀錄 - 拆帳的主要那筆, 底色用紅色顯示",
  ],
  "20251012_v0.4.1": [
    "[拆帳管理] 拆帳列表 - 新增退款按鈕",
    "[拆帳管理] 拆帳列表 - 執行拆帳時, 末五碼需一致且遊戲幣要計算",
    "[資產管理] 交流紀錄 - 退款類別, 整排顯示紅色字體",
    "[資產管理] 編輯交流單 - 退款類別, 開放跨行手續費的編輯",
    "[會員管理] 會員列表 - 遊戲暱稱新增刪除功能",
  ],
  "20251025_v0.4.2": [
    "[資產管理] 編輯交流單 - 時間紀錄拆為時分輸入",
    "[資產管理] 交流紀錄 - 取消訂單功能需要進行二次確認",
    "[拆帳管理] 拆帳列表 - 拆帳、退款功能, 直接完成訂單, 不需要再確認 (特規功能), 讓總餘額可以即時對上",
    "[員工管理] 業績列表 - 退款的部分, 不計算到員工業績"
  ],
  "20251102_v0.4.3": [
    "[全系統] 時間相關的輸入框, 預設值為當日",
    "[交流單管理] 新增交流單 - 遊戲幣手續費, 無條件進位",
    "[交流單管理] 新增交流單 - 超商開單超過 2000 會失敗",
    "[資產管理] 資產列表 - 總餘額跟交流紀錄最上方的餘額一致",
    "[資產管理] 交流紀錄 - 使用拆帳出來的沒有底色",
    "[員工管理] 員工結帳、員工業績 - 超商的營業額是抓實收金額",
    "[會員管理] 會員列表 - 顯示順序依照首次交流時間排序, 最舊在最上方",
    "[會員管理] 會員列表 - 將原先功能區中的功能放入畫面中",
  ],
  "20251105_v0.4.4": [
    "[資產管理] 交流紀錄 - 使用拆帳時, 直接使用建立者的班別底色",
    "[會員管理] 新增會員 - 首次交流時間直接帶入當日",
  ]
  ...
  // 20250917
  // 管理者帳號可刪除、其餘可修改 (全資料表都有可能，不侷限)
  // 只要有修改，需紀錄時間 ＆ 操作者
  // 各個資料表需顯示修改紀錄，例如編輯會員等畫面，且直接顯示所有紀錄在最下方
  // 結帳: 各遊戲顯示超商手續費、免手續幣 (跟員工畫面一樣的)
  // 晚班當天有上兩班，結帳時間以中午 12:00 做切割點
  // 員工業績列表 9/15 手續費應該要是 -5，免手續幣應該是 4550
  // 當日結帳 需要補上
})
